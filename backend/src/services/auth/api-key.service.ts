...
export class ApiKeyService {
  ...
  async generateApiKey(
    ...
    if (!user) {
      this.logger.warn(`API key generation attempt for non-existent user ID: ${userId}`);
    ...
    this.logger.log(`API key generated for user: ${userId}, key ID: ${apiKey.id}, scopes: ${scopes.join(',')}`);
  ...
  async validateApiKey(rawKey: string): Promise<{ user: User; apiKey: ApiKey } | null> {
    ...
    this.logger.log(`API key validated successfully for user: ${apiKey.user.id}, key ID: ${apiKey.id}`);
    return { user: apiKey.user, apiKey };
  }
  
  async hasRequiredScopes(apiKey: ApiKey, requiredScopes: string[]): Promise<boolean> {
  ...
  async revokeApiKey(apiKeyId: string): Promise<void> {
    const result = await this.apiKeyRepository.update(apiKeyId, { active: false });
  ...
  async rotateApiKey(
    ...
    if (existingKey.user.id !== userId) {
      this.logger.warn(`API key rotation failed: user ${userId} attempted to rotate key ${apiKeyId} owned by ${existingKey.user.id}`);
