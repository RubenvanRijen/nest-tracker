...
export class AuthController {
  ...
  async twoFaLogin(@Body() body: TwoFactorAuthVerifyDto) {
    ...
    if (!user || !user.twoFaSecret) {
      this.logger.warn(
        `2FA login attempt for user without 2FA enabled: ${body.email}`,
      );
    ...
    schema: {
      properties: {
        ...
        otpauthUrl: {
          type: 'string',
          example:
            'otpauth://totp/NestTracker:user@example.com?secret=ABCDEFGHIJKLMNOPQRSTUVWXYZ234567&issuer=NestTracker',
        },
      },
    },
  })
  ...
  @ApiUnauthorizedResponse({
    description: 'Unauthorized - invalid token or no pending setup',
  })
  async enable2fa(
    ...
    if (!user.pendingTwoFaSecret) {
      this.logger.warn(
        `2FA enable attempt without pending setup for user: ${user.id}`,
      );
  ...
  twofaStatus(@Req() req: IAuthRequest) {
    ...
    return {
      ...
      hasBackupCodes: !!(
        ...
        user.twoFaBackupCodes.length > 0
      ),
    };
  }
  ...
  @ApiUnauthorizedResponse({
    description: 'Unauthorized - invalid token or 2FA not enabled',
  })
  async rotate2fa(
    @Req() req: IAuthRequest,
    @Body() body: TwoFactorAuthVerifyDto,
    ...
    if (!user.twoFaSecret) {
      this.logger.warn(
        `2FA rotation attempt for user without 2FA enabled: ${user.id}`,
      );
      throw new UnauthorizedException('2FA not enabled for this user');
    }
    ...
    if (!valid) {
      this.logger.warn(
        `Failed 2FA verification for rotation attempt: ${user.id}`,
      );
      throw new UnauthorizedException('Invalid 2FA token');
    }

    // Generate new secret
    const { secret, otpauthUrl } = this.twoFaService.rotate2faSecret(
      user.email,
    );
  ...
  async generateBackupCodes(
    ...
    if (!user.twoFaSecret) {
      this.logger.warn(
        `Backup code generation attempt for user without 2FA enabled: ${user.id}`,
      );
      throw new UnauthorizedException('2FA not enabled for this user');
    }
    ...
    if (!valid) {
      this.logger.warn(
        `Failed 2FA verification for backup code generation: ${user.id}`,
      );
      throw new UnauthorizedException('Invalid 2FA token');
    }
  ...
  @ApiOkResponse({
    ...
    schema: {
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Login successful using backup code',
        },
  ...
  async verifyBackupCode(@Body() body: TwoFactorAuthBackupDto) {
    ...
    if (
      ...
      this.logger.warn(
        `Backup code verification attempt for user without valid backup codes: ${body.email}`,
      );
      throw new UnauthorizedException(
        'No backup codes available for this user',
      );
    }

    // Verify the backup code
    const codeIndex = this.twoFaService.verifyBackupCode(
      body.backupCode,
      user.twoFaBackupCodes,
    );
    ...
    user.twoFaBackupCodes = user.twoFaBackupCodes.filter(
